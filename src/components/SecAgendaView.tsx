...
export function SecAgendaView({ onSelectPatient }: { onSelectPatient?: (id: string) => void }) {
  ...
  const [newAppt, setNewAppt] = useState({ patientName: "", patientId: "", type: "Control", time: "08:00", duration: 1, reason: "" });
    ...
    for (let d = 1; d <= totalDays; d++) {
      ...
      days.push(
        ...
        >
          {d}
          {hasAppt && (
            <div style={{ position: "absolute", bottom: 4, width: 3, height: 3, borderRadius: "50%", background: isSelected ? "#1a0e00" : accent }} />
          )}
        </button>
      );
    }
  ...
  useEffect(() => {
    ...
    const search = async () => {
      ...
      try {
        ...
        setPatientResults(results.filter((p: any) => p.name.toLowerCase().includes(patientSearch.toLowerCase())).slice(0, 10));
      } catch (error) {
      ...
      try {
        ...
        const mapped = list.map((a: any) => {
          ...
          return {
            ...
            color: a.type === "Urgency" ? danger : (a.type === "Control" ? accentB : (a.type === "Prenatal" ? "#A78BFA" : accent))
          };
        });
        ...
        <div style={{ padding:"20px 24px", borderBottom:`1px solid ${glass.border}`,
              ...
              {showDoctorList && (
                <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, zIndex: 210, width: 220, background: "rgba(18,18,26,0.98)", border: `1px solid ${glass.borderS}`, borderRadius: 16, backdropFilter: "blur(24px)", boxShadow: "0 16px 40px rgba(0,0,0,0.5)", overflow: "hidden" }}>
                  ...
                  <div style={{ maxHeight: 300, overflowY: "auto" }}>
                    <button 
                      onClick={() => { setSelectedDoctor(null); setShowDoctorList(false); }}
                      style={{ width: "100%", padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, background: !selectedDoctor ? glass.navAct : "none", border: "none", borderBottom: `1px solid ${glass.border}44`, cursor: "pointer", textAlign: "left", transition: "background 0.2s" }}
                    >
                      ...
                      <span style={{ fontSize: 12, fontWeight: !selectedDoctor ? 700 : 500, color: !selectedDoctor ? accent : text1 }}>All Doctors</span>
                    </button>
                    {doctors.map(doc => (
                      <button 
                        key={doc.id} 
                        onClick={() => { setSelectedDoctor(doc); setShowDoctorList(false); }}
                        style={{ width: "100%", padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, background: selectedDoctor?.id === doc.id ? glass.navAct : "none", border: "none", borderBottom: `1px solid ${glass.border}44`, cursor: "pointer", textAlign: "left", transition: "background 0.2s" }}
                      >
                        ...
                        <span style={{ fontSize: 12, fontWeight: selectedDoctor?.id === doc.id ? 700 : 500, color: selectedDoctor?.id === doc.id ? accent : text1 }}>{doc.name}</span>
                        {!doc.isPlaceholder && (
                          <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#10b981", marginLeft: "auto", boxShadow: "0 0 8px #10b98188" }} title="Account connected" />
                        )}
                      </button>
                    ))}
                  </div>
          ...
          <div style={{ display:"flex", gap:8, position: "relative" }}>
            ...
            <button onClick={() => setShowModal(true)} style={{ display:"flex", alignItems:"center", gap:6, fontSize:11, fontWeight:700,
              padding:"8px 16px", borderRadius:10, background:accent, color:"#1a0e00", border:"none", cursor:"pointer" }}>
            ...
            {appts.map((a, i) => {
              ...
              return (
                <div key={a.id} 
                  ...
                  <p style={{ fontSize:12, fontWeight:800, color:text1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.name}</p>
                  <p style={{ fontSize:10, fontWeight: 600, color: a.color }}>{a.type} • {a.dur}h</p>
                </div>
              );
            })}
          </div>
          ...
          <GCard style={{ width:400, padding:24, border:`1px solid ${glass.borderS}` }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
              <div>
                ...
                {selectedDoctor && <p style={{ fontSize: 11, color: accent, fontWeight: 600 }}>For: Dr. {selectedDoctor.name}</p>}
              </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              <div>
                ...
                <input value={patientSearch} onChange={e => { setPatientSearch(e.target.value); setShowPatientResults(true); }}
                ...
                {showPatientResults && patientResults.length > 0 && (
                  <div style={{ position:"absolute", zIndex:1000, background:"#12121a", border:`1px solid ${glass.border}`, borderRadius:12, width:"calc(100% - 48px)", maxHeight:150, overflowY:"auto" }}>
                    {patientResults.map(p => (
                      <button key={p.id} onMouseDown={() => { setNewAppt({...newAppt, patientName: p.name, patientId: p.id}); setPatientSearch(p.name); setShowPatientResults(false); }}
                        style={{ width:"100%", padding:10, background:"none", border:"none", borderBottom:`1px solid ${glass.border}`, color:text1, textAlign:"left", cursor:"pointer" }}>
                        {p.name}
                      </button>
                    ))}
                  </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                <div>
                  <p style={{ fontSize:11, fontWeight:700, color:text1, marginBottom:6, textTransform:"uppercase" }}>Time</p>
                  <select value={newAppt.time} onChange={e => setNewAppt({...newAppt, time: e.target.value})}
                    style={{ width:"100%", padding:"10px 12px", background:"#1a1a24", border:`1px solid ${glass.border}`, borderRadius:10, color:text1, outline:"none" }}>
                    {timeOptions.map(t => <option key={t} value={t} style={{ background: "#1a1a24", color: text1 }}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <p style={{ fontSize:11, fontWeight:700, color:text1, marginBottom:6, textTransform:"uppercase" }}>Duration (h)</p>
              ...
              <div>
                <p style={{ fontSize:11, fontWeight:700, color:text1, marginBottom:6, textTransform:"uppercase" }}>Reason for Appointment</p>
                <textarea 
              ...
              <button onClick={handleAddAppointment} disabled={isSaving || !newAppt.patientName} style={{ marginTop:8, padding:"12px", borderRadius:10, background:accent, color:"#1a0e00", fontWeight:700, border:"none", cursor:(isSaving || !newAppt.patientName)?"not-allowed":"pointer", opacity:(isSaving || !newAppt.patientName)?0.7:1 }}>
